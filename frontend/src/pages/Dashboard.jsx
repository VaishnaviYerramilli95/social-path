      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
        ...
        >
          <div className="flex flex-col gap-3.5 mt-2">
            {[
              { name: 'Facebook Page', detail: '@SocialPilotDev', status: 'Connected', icon: IoLogoFacebook, color: 'text-[#1877F2] bg-[#1877F2]/5' },
              { name: 'Instagram Business', detail: 'socialpilot_dev', status: 'Connected', icon: IoLogoInstagram, color: 'text-[#E4405F] bg-[#E4405F]/5' },
              { name: 'LinkedIn Company', detail: 'SocialPilot Inc', status: 'Connected', icon: IoLogoLinkedin, color: 'text-[#0A66C2] bg-[#0A66C2]/5' },
              { name: 'Twitter Profile', detail: '@SocialPilotHQ', status: 'Connected', icon: IoLogoTwitter, color: 'text-slate-800 dark:text-slate-100 bg-slate-500/5' }
            ].map((account) => (
              <div
                key={account.name}
              ...
              >
                <div className="flex items-center gap-3">
                  <span className={`w-9 h-9 rounded-xl flex items-center justify-center text-lg ${account.color}`}>
                    <account.icon />
                  </span>
                  <div>
                    <h5 className="text-xs font-bold text-slate-700 dark:text-slate-200">{account.name}</h5>
                    <p className="text-[10px] text-slate-400 dark:text-slate-500 font-medium mt-0.5">{account.detail}</p>
                  </div>
                </div>
        ...
        >
          <div className="flex flex-col gap-4 mt-2">
            {[
              { id: '1', log: "LinkedIn post for 'Summer Launch 2026' campaign was successfully published.", time: "10 minutes ago", icon: IoCheckmarkDoneOutline, color: 'text-emerald-500 bg-emerald-50 dark:bg-emerald-950/20' },
              { id: '2', log: "Campaign budget limit reached warning: 'Summer Product Launch' is at 95% threshold.", time: "1 hour ago", icon: IoAlertCircleOutline, color: 'text-amber-500 bg-amber-50 dark:bg-amber-950/20' },
              { id: '3', log: "Scheduled Instagram post for tomorrow was updated by Jane Doe.", time: "4 hours ago", icon: IoCalendarClearOutline, color: 'text-blue-500 bg-blue-50 dark:bg-blue-950/20' },
              { id: '4', log: "New Twitter Account linked to workspace.", time: "1 day ago", icon: IoAddOutline, color: 'text-primary-500 bg-primary-50 dark:bg-primary-950/20' }
            ].map((activity) => (
              <div
                key={activity.id}
                className="flex items-start gap-3.5 p-3 rounded-xl border border-slate-50 dark:border-dark-700/20"
              >
                <span className={`w-8 h-8 rounded-lg flex items-center justify-center text-sm flex-shrink-0 ${activity.color}`}>
                  <activity.icon />
                </span>
                <div className="flex-1 min-w-0">
