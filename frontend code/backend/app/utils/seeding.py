...
def seed_user_data(db: Session, user_id: str):
    ...
    for cmp in campaigns_data:
        ...
        db.add(db_cmp)
        db_campaigns.append(db_cmp)
    db.commit()

    # 4. Seed Posts / Scheduler Items
    posts_data = [
        {
            "id": "pst_1",
            "platform": "linkedin",
    ...
    for pst in posts_data:
        ...
        db.add(db_pst)
        db_posts.append(db_pst)
    db.commit()

    # 5. Seed Analytics for Published and Scheduled/Failed Posts
    analytics_records = [
        # Post 3
